...
| A4 | Common `SeriesServices` + `AppServices`; `MainActivity` builds them; caliber defaults on `BackendTokenStore`; `FileImageCache` on kotlinx-io; delete the iOS/Android service copies. | Android gate; History thumbnails still served from `cacheDir/series` | done (2026-09-09; one commonMain `SeriesServices` + `AppServices` built in `MainActivity`; `FileImageCache` on kotlinx-io 0.7.0; native SQLDelight driver in iosMain; iOS framework links; phone check pending) |
| A5 | Competition seam (`Screen.Competition`, `CompetitionHost`, androidMain `CompetitionFlow.kt`). | Android gate; flip `SHOW_COMPETITION=true` once locally to smoke it | done (2026-09-09; `Screen.Competition` + `CompetitionHost`; androidMain `ui/competition/CompetitionFlow.kt` owns the four steps, `SHOW_COMPETITION` and the webshooter session restore; compiled once with the flag on) |
| A6 | Common zip writer + `exportSeriesZip`, kotlinx-datetime stamps, delete `java.util.zip`/`java.time` uses. | Android gate + `ZipTest`/stamp tests; phone: export opens in Files | done 2026-09-09 (164 tests; kotlinx-datetime pinned 0.6.2 — material3 links against 0.6, 0.7 breaks the iOS date picker; phone export check pending) |
| A7 | PlatformImage seams (`width/height/centerSquare/argb/toImageBitmap/encode/decode`), `vision/ImageOps.kt` + test, Android actuals, **real iOS actuals** (`UIImage` alias + CoreGraphics), `MarkeraSnapshotViewModel` common. | Android gate; `/eval` unaffected; Mac link | done 2026-09-09 (167 tests; mosaic seed 42 identical; Mac link OK; iOS decodeSeriesJpeg does a full decode + one scaled draw — ImageIO thumbnail is the upgrade if memory bites; phone scan check pending) |
| A8 | Move `ui/**` (minus competition) to commonMain: `TargetScanController`, `FrameSource`, `CameraPermissionState` common; `rememberFrameSource`/`rememberCameraPermission`/`rememberSignIn` expects with Android actuals = existing code; drawText via `TextMeasurer`; common `BackHandler`; icons + viewmodel-compose deps; delete `App.kt`; `MainViewController` → `AppNavHost` with minimal iOS actuals so it compiles. Split into A8a (moves + expects) and A8b (drawText/backhandler/icons) if the diff passes ~1500 lines. | Android gate; phone scan screenshot before/after (overlay text now Skia); Mac link; simulator shows Home with strings, icons, nav, snackbar | done 2026-09-10 (167 tests; sim shows Home/Historik/Markera with strings, icons, nav; viewmodel-compose = org.jetbrains.androidx.lifecycle 2.9.5 — androidx has no iOS variant; viewModel() needs a factory lambda on iOS; sync script now ships staged renames; phone scan screenshot pending) |
| B1 | iOS `PhotoPickerFrameSource` + permission actual; `HoleModel` interface; real `HoleDetector.ios`; `MainViewController(holeModel)`. | sim: addmedia a target JPEG → Välj foto → Detektera → frozen frame | done 2026-09-10 (167 tests; Mac link OK; sim: Välj foto → PHPicker → dataset photo shown → Detektera freezes the frame with the results card; loadObjectOfClass(UIImage) does not map through cinterop → loadDataRepresentationForTypeIdentifier("public.image") + UIImage(data:); holes need B2) |
| B2 | Swift `HoleModel.swift` on ORT SPM + wiring. | sim log `raw=N kept=M`; hole count matches `/eval` for the same image | done 2026-09-10 (167 tests; sim: OrtHoleModel over the ORT Swift package, inference 1.3 s, raw=5 kept=5 on IMG20250530132856 = eval; the Mac clone needs best.onnx copied to composeApp/src/androidMain/assets/ for the xcodegen resource; digits=0 until B3) |
| B3 | Vision `DigitDetector.ios`. | sim: digits found, centre + ring drawn on the photo, scores shown | done 2026-09-10 (167 tests; Mac link clean first try; sim on IMG20250530132856: 13 observations → 7 digits in 1.3 s, centre=LINE_INTERSECTION, ring=true, scores [X,10,9,9,9] = 47, ring/cross/letters drawn) |
| B4 | iOS series wiring: `suspend` UserDefaults store, caches dir, `UIActivityViewController` share, dev-auth fallback + Info.plist reads; local dev server recipe in README. | sim vs local server: sign in (dev), scan → Spara → caliber dialog → saved; Historik thumbnail; delete; Serie move/save; Statistik; export share sheet | done 2026-09-10 (167 tests; sim vs dev backend in Docker `markera-dev` on 127.0.0.1:8091 (the server Gradle build needs JDK 17 the Mac lacks — run the built image instead): Apple sign-in error 1000 → dev auth "Inloggad via Dev", scan → Spara → kaliber 22lr → series 1 with image on the server, Historik thumbnail, Serie detail, Statistik, export share sheet with the 3.4 MB zip; popoverPresentationController needs `import platform.UIKit.popoverPresentationController`) |
| B5 | iOS `CameraFrameSource` (AVFoundation) with the picker icon overlay. **Compile-only + review; no device.** | Mac link; simulator still uses the picker | done 2026-09-10, compile-only (167 tests; sim + device links clean; simulator still picks photos, scan log unchanged; AVCaptureConnection.videoOrientation/isVideoOrientationSupported are plain members — no import; device behaviour unverified, no iPhone) |
...
| C4 | fastlane `beta` lane + `Appfile` (blocked on the user's App Store Connect API key + App ID). | `fastlane beta` once the key exists | done 2026-09-10 — build 1.5.0 (6) on TestFlight via `ssh -t macmini bash source/Markera/scripts/mac-beta.sh` (keychain unlock must happen in the same interactive session; the lane enabled Sign in with Apple on the App ID itself; first upload was rejected with ITMS-90208 until the deployment target matched the ORT xcframework's MinimumOSVersion 15.1, and xcodegen needed explicit CFBundleShortVersionString/CFBundleVersion) |
...
| D3 | App Store listing: `fastlane/metadata` (Swedish texts from the Play listing, category, copyright, review contact, full age-rating questionnaire), four 6.9" simulator screenshots, `metadata`/`release`/`dedupe_screenshots` lanes, Deliverfile; iOS step in `/release`. | listing visible on ASC 1.5.0 | done 2026-09-10 (listing + screenshots on the editable 1.5.0; App Privacy form + Submit left to the user) |
