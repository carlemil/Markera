...
/**
 * Compares the recovered black-7-ring calibration against a tightened selector
 * that anchors blob choice to the viewfinder guide-circle prior (expected
 * radius + centre). Writes one annotated 3x3 mosaic set per selector plus a
 * per-image comparison, so we can see how much the false positives and NO-FITs
 * improve. Mirrors the app's square capture (centre-crop + downscale).
 *
...
class BlackRingMosaicTest {
    ...
    private fun processOne(file: File): TileResult {
        ...
        // Seed = image centre (= guide-circle centre after the square capture);
        // on-device this can be replaced with the digit-line centre.
