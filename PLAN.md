...
| 31 | Server + admin: store the target geometry per series — digit-row centre and the 6/7 ring ellipse (`geometry: {centreX, centreY, ringCx, ringCy, ringSemiMajor, ringSemiMinor, ringRotationRad}`, source-image px, nullable, on POST/PUT/GET/admin); admin photo draws the centre and the ring; moving/adding a hole recomputes `distanceMm` from the geometry (JS port of the `scoreHits` un-projection) instead of nulling it (requested 2026-09-07) | done 2026-09-07, deployed — browser check of the recomputed mm pending a series with geometry (task 32) |
| 32 | App: send the geometry with every scan (`onSeriesDetected` gets centre + ring), draw centre + ring on the detail photo, recompute `distanceMm` on drag from a shared pure `distanceMm(x, y, centre, ring)` in `vision/` that `scoreHits` also uses; old series without geometry keep nulling (requested 2026-09-07) | done 2026-09-07, installed on the phone — **awaiting user**: scan a real target once (Detektera → Spara), then check the detail screen (ring + centre drawn, drag keeps a mm value) and the admin page for that series |
