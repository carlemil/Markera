...
| 28a | Server: `PUT /series/{id}` (owner only, body = the POST body, replaces caliber/timestamp/holes → 204) + `detectedX`/`detectedY` on holes (migration + backfill from x/y where detected_ring is set); admin kind shows "moved" | queued |
| 28b | App: series detail screen from a history row — photo with markers, time, caliber, total, hole list (score, mm, kind); tap a score to edit it, drag a marker to move it; Spara → PUT; history refreshes | queued |
| 28c | Admin web: the series page becomes editable — score inputs per hole, drag a marker to move it, click the photo to add a hole, delete per row, Save → `PUT /admin/series/{id}` (Basic auth; same body/logic as the app PUT) | queued |
