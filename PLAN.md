...
| 80 | Historik: changing a filter scrolls the list back to the newest series — today clearing the 9mm chip left the list anchored on the 9mm cards mid-list (found on the phone 2026-09-15, follow-up to 77) | queued |
| 81 | Statistik Tavla: two knobs on the age colour bar pick a start and end series within the filtered range; they snap to the series positions, each series is a small dot on the bar; the target, the mean/median markers and the measurement rows show only the picked segment, hits keep their full-range colours; the dates under the bar follow the knobs (requested 2026-09-15) | queued |
