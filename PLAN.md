...
| 42 | App: mirror the admin delete on the series detail screen — a visible "Radera" action (top bar or beside Spara) with the existing `history_delete_*` confirm dialog, `api.deleteSeries`, then back to a history list that no longer shows the series (long-press delete in the list stays) (requested 2026-09-07) | queued after 41 |
