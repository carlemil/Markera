...
| 42 | App: mirror the admin delete on the series detail screen — a visible "Radera" action (top bar or beside Spara) with the existing `history_delete_*` confirm dialog, `api.deleteSeries`, then back to a history list that no longer shows the series (long-press delete in the list stays) (requested 2026-09-07) | done — gate green; phone check pending (phone was disconnected) |
| 43 | Statistik screen polish: "Välj…" chip moves onto the hits row; − and + become real icon buttons placed left of the "N träffar" text; tighter spacing between the filter rows; ring digits 5–9 also drawn along the vertical axis of the target (requested 2026-09-07) | done — gate green; phone check pending (phone was disconnected) |
