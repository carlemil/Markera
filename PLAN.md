...
| 47 | Scores only from hole position: remove score editing by tapping the digit/picker (scan screen, series detail, wizard?) — a score changes only by moving a detected hole or adding + positioning a new one, then it is computed from the geometry, so every saved score is accurate for training (requested 2026-09-07) | queued |
| 48 | Statistik screen: a "?" (top bar action) opening a dialog that explains each value shown (mean distance, hole-to-hole, group size, mean/median point of impact, tens share, best/worst, colour scale) (requested 2026-09-07) | queued |
| 49 | Statistik: the mean and median markers in the target image must be the same size and style as in the legend (requested 2026-09-07) | queued |
| 50 | Statistik: widen the date colour scale of the hits and make it pop against the white/black target (requested 2026-09-07) | queued |
