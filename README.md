...
The same Compose UI and scoring pipeline run on iOS: the ONNX model through the
ONNX Runtime Swift package (`iosApp/iosApp/HoleModel.swift`), ring digits through
Apple Vision OCR, the camera through AVFoundation (a photo picker stands in on the
simulator). The Xcode project is generated by xcodegen from `iosApp/project.yml`;
`iosApp/README.md` has the build, simulator and TestFlight recipes. So far it has
only been verified on the simulator.
